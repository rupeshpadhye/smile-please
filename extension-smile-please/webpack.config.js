...
const env = dotenv.config({path: __dirname + `/${nodeEnv}.env`});
...
module.exports = {
	...
	plugins: [
		...
		new HtmlWebpackPlugin({
			...
			title: 'Smile Please - Options Page',
			chunks: ['options'],
		}),
		new HtmlWebpackPlugin({
		...
		new CopyWebpackPlugin([
			...
			// },
			{
				from: path.join(sourceRootPath, '128.png'),
				to: path.join(distRootPath, '128.png'),
				test: /\.(jpg|jpeg|png|gif|svg)?$/,
			},
			{
